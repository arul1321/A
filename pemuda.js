...
pemuda.sendMessage(from, img, image, { quoted: mek, thumbnail: fs.readFileSync('./pemuda_1.jpg')})
break
case 'waifu':
reply(mess.wait)
...
pemuda.sendMessage(memekkkk, 'Wkwkwk', MessageType.text)
break
				case 'script':
		case 'sc':
		case 'sourcecode':
...
break
		...
		try {
					ppimg = await pemuda.getProfilePicture(anu)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				ano = await pemuda.getProfilePicture(anu)
...
pemuda.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true, quoted: mek})
fs.unlinkSync(ran)
})
break
case 'gemuk':
...
if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	...
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  pemuda.sendMessage(from, anu, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true})
	  }
...
case 'matrix':
		...
		const encmediia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	        const meidia = await pemuda.downloadAndSaveMediaMessage(encmediia, `./sticker/${sender}`)
		    exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
		    if (error) return reply('error')
		    pemuda.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), MessageType.sticker, {quoted: mek})
...
case 'tts':
					...
					bby.length > 300
						...
						: gtts.save(ranm, bby, function () {
							exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
								...
								pemuda.sendMessage(from, buff, audio, { duration: 359996400, ptt: true, quoted: mek })
								fs.unlinkSync(rano)
							})
						})
					break
		    ...
		    pemuda.groupDemoteAdmin(from, [demote])
		    ...
		    pemuda.groupMakeAdmin(from, [promote])
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						...
						buff = await pemuda.downloadMediaMessage(encmedia)
						pemuda.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400})
						}
...
if (isMedia && !mek.message.videoMessage || isQuotedAudio) {
						const encmedia = isQuotedAudio ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await pemuda.downloadMediaMessage(encmedia)
						pemuda.sendMessage('status@broadcast', buff, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt: true})
						}
...
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
                    ...
                    pemuda.sendMessage('status@broadcast', buffer3, MessageType.video, {duration: 359996400, caption: `${konti}`})
...
const sname = pemuda.contacts[ambl] != undefined ? pemuda.contacts[ambl].notify = undefined ? PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international') : pemuda.contacts[ambl].notify || pemuda.contacts[ambl].vname : PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international')
reply(sname)
break
				case 'getpict':
				case 'getpic':
...
case 'tagall':
            ...
            if (!isGroup) return reply(mess.only.group)
				...
				pemuda.sendMessage(from, cs[2], MessageType.text, target)
				break
              case 'hacked':
              if (!isGroup) return reply(mess.only.group)
              if (args.length < 1) return reply('Teksnya?')
              reply('Otw Hack')
                ...
                await sleep(1000)
                pemuda.groupUpdateDescription(from, `_${pushname} telah meretas grup ini_`)             
                await sleep(1000)
...
reply(`${ngazap(prefix)}`)
break
//By Xchildss X YuzzuKamiyaka x Pemida Teknologi
case 'virtex':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
...
//--------------Pemuda Teknologi-----------\\
  case 'jadivirus':
            ...
            ini_buffer = fs.readFileSync(file)
               pemuda.sendMessage(from, ini_buffer, sticker, { thumbnail : virgam, ftoko})
            fs.unlinkSync(file)
            } else if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
            ...
            ini_buffer = fs.readFileSync(file)
            pemuda.sendMessage(from, ini_buffer, image, { thumbnail : virgam, ftoko, options})
            fs.unlinkSync(file)
        } else if ((isMedia && !mek.message.videoMessage || isQuotedAudio) && args.length == 0) {
            ...
            ini_buffer = fs.readFileSync(file)
               pemuda.sendMessage(from, ini_buffer, audio, { thumbnail : virgam, ftoko})
            fs.unlinkSync(file)
        }  else if ((isMedia && !mek.message.videoMessage || isQuotedVideo) && args.length == 0) {
            ...
            ini_buffer = fs.readFileSync(file)
          pemuda.sendMessage(from, ini_buffer, video, { thumbnail : virgam, ftoko})
            fs.unlinkSync(file)
        } else{
  ...
  case 'jadivirgam':
   ...
   encmediaz = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					...
					ran = getRandom('.jpg')
						...
						if (err) return ephe('Error!')
						hah = fs.readFileSync(ran)
					pemuda.sendMessage(from, hah, audio, {mimetype: 'image/jpg', ptt:true, duration: 359996400, quoted:mek})
						fs.unlinkSync(ran)
...
"descriptionCount": "999999999",
  ...
  break
    ...
    hmm4 = fs.readFileSync(`./pemuda_1.jpg`)
imeu = await pemuda.prepareMessage('0@s.whatsapp.net', hmm4, image)
...
case 'x':
            ...
            "externalAdReply": {
                ...
                "thumbnail": fs.readFileSync(`./pemuda_1.jpg`),
                "sourceUrl": "https://youtu.be/vhUQGd_HDEw"}}})
                break
        case 'bugbutton':
        if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
        pemuda.toggleDisappearingMessages(from, 0)
        sendButLocation(from, `${ngazap(prefix)}`, `${virtag(prefix)}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}bbaij72njnwjibdo16830nslm1782`,buttonText:{disText:'⚔️ Nanda Style ⚔️'},type:1}])
        pemuda.toggleDisappearingMessages(from, 0)
break
case 'bugrow':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
   			...
   			var mulaikah = aramat[0].url							
                  try {
                   ...
                   break
                            ...
                            const mulaihah = i.data.result[0].url
                            try {
                                reply(mess.wait)
                                ytv(mulaihah)
                                .then((res) => {
